...
/**
 *
 * To the poor soul who has to look at this code. You may ask yourself...why is
 * this called a Streaming controller?
 * It's because it was intended to be a streaming controller, and that's what
 * we're trying to "simulate"
 *
 * Four score ago, an ashamed developer set out to develop a streaming
 * controller that would stream accumulo results
 * straight to the http client. It would use akka's reactive streams, and it
 * would require no disk or ram backpressure
 * to accomplish. That sounds awesome right? That ashamed developer was woefully
 * unprepared for what was about to happen.
 * There were countless libraries to upgrade, conventions to change, all to get
 * to the position where it was time to
 * put in the reactive stream. Priorites and deadlines came and went, but the
 * reactive stream refused to stream, nor
 * be reactive. Only to turn out that there were numerous bugs/issues logged for
 * the "experimental / totally non-production"
 * version of reactive streams. Therefore, in an attempt to set a "future
 * raiseable standard", we have this hunk of junk that
 * writes a file, "streams" it, and then deletes the file. So whoever gets to
 * refactor this when reactive streams is ready
