...
import com.dataprofiler.util.objects.ui.SearchObject;
import com.dataprofiler.util.objects.ui.SearchElement;
...
public class AccumuloControllerMultiSearch extends Controller {
  ...
  public Result multiSearch(Request req)
    ...
    logger.info(
        "Multi-search Metadata section: {} ms",
        (endTime - startTime) / 1000000);

    Integer limit = spec.getLimit();

    // List<SearchObject> result = new ArrayList<>();
    Map<String, SearchObject> result = new HashMap<>();
    ...
    for (int i = 0; i < numTerms; i++) {
      ...
      try (ClosableIterator<ColumnCountIndexObject> iter = new ColumnCountIndexObject().find(context, spec)
        ...
        while (iter.hasNext() && result.size() < limit) {
          ...
          System.out.println("Value being added: " + indexObj.getValue());
          System.out.println("normalized value: " + indexObj.getNormalizedValue());

          System.out.println("Value count: " + indexObj.getCount());

          if (i == 0) {
            // System.out.println("Value being added: " + indexObj.getValue());
            // System.out.println("Value count: " + indexObj.getCount());
            result.put(indexObj.getValue(), new SearchObject(indexObj));

            // System.out.println("result size: " + result.size());
            // result.add(new SearchObject(indexObj));
    ...
    endTime = System.nanoTime();
    logger.info(
        "Multi-Search Accumulo Section: {} results in {} ms",
        result.size(),
        (endTime - startTime) / 1000000);
    return ok(Json.toJson(result.values()));
  }
  ...
  // req.attrs().get(RulesOfUseHelper.USER_CONTEXT_TYPED_KEY);
  // VersionedDataScanSpec spec = new VersionedDataScanSpec(context,
  // unversionedSpec);
  ...
  // Map<String, Map<String, ColumnCountIndexObject>> termLevelCounts = new
  // HashMap<>();
  // List<ColumnCountIndexObject> result = new ArrayList<>();
  // Set<String> intersection_curr = new HashSet<>();
  // Set<String> intersection_prev = new HashSet<>();

  // List<String> originalTerms = new ArrayList<>(spec.getTerm().subList(0,
  // spec.getTerm().size() - 1));
  ...
  // try (ClosableIterator<ColumnCountIndexObject> iter = new
  // ColumnCountIndexObject().find(context, spec)
  // .closeableIterator()) {
  // while (iter.hasNext() && result.size() < limit) {
  // ColumnCountIndexObject indexObj = iter.next();
  ...
  // termLevelCounts.forEach(
  // (k, v) -> v.keySet().removeIf(kInner ->
  // !intersection_prev.contains(kInner)));
  ...
  // last -> {
  // merged.merge(
  // last + level, buildResultElement(index),
  ...
  // prefix + term + level, buildResultElement(index),
  // AccumuloControllerMultiSearch::merge));
  // });
  ...
  // (termAndLevel, v) -> {
  // try {
  // String[] values = mapper.readValue(v.get("value").toString(),
  // String[].class);
  ...
  // termGrouping.entrySet().removeIf(e -> e.getKey().split("\\|\\|").length !=
  // numTerms);
