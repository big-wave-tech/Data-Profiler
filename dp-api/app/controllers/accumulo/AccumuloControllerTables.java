...
@Authenticated
@AccumuloUserContext
public class AccumuloControllerTables extends Controller {
  ...
  @Inject
  public AccumuloControllerTables(Config config) throws IOException {
    this.config = config;
  ...
  public Result searchTableNames(Request req) throws IOException {
    ...
    try (ClosableIterator<VersionedMetadataObject> iter = new VersionedMetadataObject()
        .searchTableNames(context, context.getCurrentMetadataVersion(), terms, startsWith)
        .setBatch(true)
        .closeableIterator()) {
    ...
    logger.info(
        "searchTableNames: {} results in {} ms",
        results.size(),
