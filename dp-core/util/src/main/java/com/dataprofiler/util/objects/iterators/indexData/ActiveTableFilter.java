package com.dataprofiler.util.objects.iterators.indexData;
...
public class ActiveTableFilter extends Filter {
  ...
  /**
   * Utility method for getting the prefix from the column qualifier value of a
   * GLOBAL index entry.
   * This grabs everything up to, but excluding, the second null byte in a byte
   * string.
   *
   * @param columnQualifier
   * @return a substring up to the second null byte, or null if 2 null bytes are
   *         not found.
   */
  ...
  public void init(
    ...
    if (options.containsKey(CONF_ACTIVE_TABLES)) {
      this.filters = ImmutableSet.copyOf(
