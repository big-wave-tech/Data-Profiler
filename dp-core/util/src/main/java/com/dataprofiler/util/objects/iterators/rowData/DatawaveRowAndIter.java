package com.dataprofiler.util.objects.iterators.rowData;
...
public class DatawaveRowAndIter implements SortedKeyValueIterator<Key, Value> {
  ...
  /**
   * The value to return `{"col_0":"val_0", "col_1":"val_1", ...,
   * "col_n":"val_n"}`
   */
  ...
  private DatawaveRowAndIter(DatawaveRowAndIter other, IteratorEnvironment env) {
    ...
    if (other.sources != null) {
      ...
      for (int i = 0; i < sourcesCount; i++) {
        sources[i] = new ColumnValueSource(
  ...
  public void init(
    ...
    for (int i = 1; i < colValues.length; i++) {
      sources[i] = new ColumnValueSource(source.deepCopy(env), colNames[i], colValues[i], notFlag[i]);
    }
  ...
  public void next() throws IOException {
    ...
    // precondition: the current row is set up and the sources all have the same
    // column qualifier
    // while we don't have a match, seek in the source with the smallest column
    // qualifier
    sources[0].iter.next();
  ...
  public void seek(Range range, Collection<ByteSequence> collection, boolean b) throws IOException {
    ...
    // Seek each of the sources to the right column family within the row given by
    // key
    for (int i = 0; i < sourcesCount; i++) {
      Key sourceKey;
  ...
  private void advanceToIntersection() throws IOException {
    ...
    while (cursorChanged) {
      // seek all of the sources to at least the highest seen column qualifier in the
      // current row
      cursorChanged = false;
      for (int i = 0; i < sourcesCount; i++) {
        if (currentPartition == null) {
  ...
  private boolean seekOneSource(int sourceID) throws IOException {
    ...
    if (sources[sourceID].notFlag) {
      while (true) {
        ...
        // If the source is before the current partition, seek to the correct RowID and
        // ColF
        if (partitionCompare > 0) {
          // seek to at least the currentRow
          Key seekKey = buildKey(currentPartition, sources[sourceID].colFam, sources[sourceID].colVal);
        ...
        if (sources[sourceID].colVal != null) {
          ...
          int colValCompare = sources[sourceID].colVal.compareTo(getColValue(sources[sourceID].iter.getTopKey()));

          // If the source is before the current value seek to the required value
          if (colValCompare > 0) {
            Key seekKey = buildKey(
                currentPartition,
        ...
        // Ensure the source is at the correct RowUID (UID for the row in the relational
        // table)
        ...
        // Force the entire process to go to the next row because this is an invalid
        // result
        sources[0].iter.next();
        advancedCursor = true;
        break;
      ...
      while (true) {
        ...
        // If the compare is positive, the current partition is past the source's
        // partition
        ...
        // If the compare is negative, the source's partition is past the current
        // partition
        // Advance the cursor
        if (partitionCompare < 0) {
        ...
        if (sources[sourceID].colVal != null) {
          ...
          // If the required value is before the source's current value, seek to the
          // required value
          if (colValCompare > 0) {
  ...
  public static class ColumnValueSource {
    ...
    ColumnValueSource(
      ...
      this.seekColfams = Collections.singletonList(
