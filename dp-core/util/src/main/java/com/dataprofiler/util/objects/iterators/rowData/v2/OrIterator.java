...
public class OrIterator implements IndexIterator {
  ...
  private final PriorityQueue<IndexIterator> minHeap = new PriorityQueue<>(
      Comparator.comparing(SortedKeyValueIterator::getTopKey));
  ...
  public void next() throws IOException {
    ...
    while (!minHeap.isEmpty()) {
      ...
      if (bufferA.equals(bufferB)) {
        ...
        current.next();
        if (current.hasTop()) {
          // if we still have some data left, advance the iterator and push it back on the
          // queue
          minHeap.add(current);
        }
      } else {
  ...
  public void seek(Range range, Collection<ByteSequence> collection, boolean b) throws IOException {
    // reset the internal state when we're told to re-seek. typically means a
    // top-level
    // iterator reset
    this.minHeap.clear();
