...
import com.dataprofiler.util.querylang.EvaluatingVisitor;
import com.dataprofiler.util.querylang.ExpressionToIteratorTree;
...
public class RowAggregator implements SortedKeyValueIterator<Key, Value> {
  ...
  private static final Collection<ByteSequence> DOC_COLF = Collections.singletonList(new ArrayByteSequence("D"));
  ...
  public void init(
    ...
    hashLargeValues = !options.containsKey(CONF_HASH_LARGE_VALUES) || Boolean.parseBoolean(
        options.get(CONF_HASH_LARGE_VALUES));

    includeVisibility = options.containsKey(CONF_INCLUDE_VISIBILITY_WITH_VALUE) && Boolean.parseBoolean(
    ...
    while (tk == null && indexReader != null && indexReader.hasTop()) {
      ...
      while (rowReader.hasTop()
          && startsWith(
              ...
              cq.getBytes())) {
        String columnName = getColName(rowReader.getTopKey(), colqBuffer, columnNameBuffer).toString();
        String columnValue = hashLargeValues
