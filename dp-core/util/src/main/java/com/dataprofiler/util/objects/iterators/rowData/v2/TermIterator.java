...
import static com.dataprofiler.util.objects.iterators.rowData.v2.Common.MAX_DOC_ID;
import static com.dataprofiler.util.objects.iterators.rowData.v2.Common.MIN_DOC_ID;
import static com.dataprofiler.util.objects.iterators.rowData.v2.Common.TWOFIDDYSIX;
...
public class TermIterator implements IndexIterator {
  ...
  /**
   * Some indirection here. This is called from `next()` and `seek()`. The
   * iterator contract
   * only specifies that an iterator may have data after `seek` or `next` is
   * called.
   *
   * <p>
   * Calling `seek` does not imply that `next` has been called, or vice-versa.
   * This is a
   * convenience method to transform the top-key into one that only has the
   * document ID in the
   * column qualifier.
   */
  public void setTopKey() {
    if (this.source.hasTop()) {
      ...
      topKey = new Key(tk.getRow(rowBuffer), Common.DOC_ID_COLUMN_FAMILY, getRowID(tk, docIDBuffer));
    }
  }
