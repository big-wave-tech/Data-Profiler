...
public class EvaluatingVisitor implements Visitor {
  ...
  public void accept(EqExpression expr) {
    this.lastResult = checkMultimap(record, expr.getColumn()).stream()
  ...
  public static void main(String[] args) throws Throwable {
    ...
    record.put("age", "32");
    Expression and = Expressions.parse(
        "{'$and': ["
    ...
    vis.reset();
    Expression expression = Expressions.parse(
        "{'$or': ["
            + "{'$eq': {'value': 'steve', 'column': 'name'}},"
            + "{'$eq': {'value': '33', 'column': 'age'}}"
            + "]}");
