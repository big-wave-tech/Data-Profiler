...
class ContainerImage:
    ...
    def deploy(self) -> bool:
        ...
        self.expose_component()
        return True


####

    def expose_component(self):
...
container_dp_ui = ContainerImage(
    ...
    options=['--build-arg',
             f'USER_FACING_API_HTTP_PATH=http://localhost:{UI_PORT}/api',
             '--build-arg',
...
# def deploy_apps(app='all') -> bool:
#     if app is None or app == 'all':
#         for component in apps.keys():
#             if not deploy_app(component):
#                 return False
#         return True
...
# def deploy_app(app):
#     logger.debug(f'Deploying: {app}')

#     # Enumerate all config files in a directory
#     conf_files = list(Path(f'{PATH_STANDALONE}/conf/{app}').glob('*.yaml'))
...
def check_minikube_status():
    (proc, _, _) = exec_cmd(['minikube', 'status'], show_output=False)
...
def is_available(name):
    proc, _, _ = exec_cmd(['which', name], show_output=False)
...
def check_minikube_environment():
    proc, _, _ = exec_cmd(['minikube', 'docker-env'])
    if not proc.returncode == 0:
        logger.info(
            'Minikube environment not set. Please run: eval `minikube docker-env`')
...
def status(args=None):
    ...
    if dir.name:
        logger.info('temp dir does exists')
        logger.info(dir.name)

    # with open('/tmp/dp-ui-port-forward.out', 'r') as ui_status_file:
